...
//[assembly: ExportRenderer(typeof(Xamarin.Forms.WebView), typeof(WebViewRenderer_fix))]
namespace SmeData.Mobile.UWP
{
	public class WebViewRenderer_fix : ViewRenderer<Xamarin.Forms.WebView, Windows.UI.Xaml.Controls.WebView>, IWebViewDelegate
	{
...
if(bases.length == 0){
    head.innerHTML = 'baseTag' + head.innerHTML;
		...
		{
			_updating = true;
			((IElementController)Element).SetValueFromRenderer(Xamarin.Forms.WebView.SourceProperty, source);
			_updating = false;
